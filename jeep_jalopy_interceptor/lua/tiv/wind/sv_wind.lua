...
local function SampleGStormsTornadoWindAt(pos)
    ...
    -- windspeedMPH is the single return value (blended windspeed).
    if not windspeedMPH or windspeedMPH ~= windspeedMPH then return nil, nil end
    if windspeedMPH <= 0 then return nil, nil end

    -- Wind direction was written into gsOutVector by the function.
    local dir = (gsOutVector:LengthSqr() > 0.01)
        and gsOutVector:GetNormalized() or Vector(1, 0, 0)

    return math.Clamp(windspeedMPH, 0, TIV.Config.WindMaxSimulated), dir
end
