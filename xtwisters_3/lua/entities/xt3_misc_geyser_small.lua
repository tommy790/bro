...
ENT.DisplayName = "Small Geyser"
ENT.PrintName = "XTwisters3 Small Large"
...
ENT.GeyserSize = 80
ENT.EruptSize = 80
ENT.EruptStrength = 95

local GeyserScale = Vector(2, 3, 1)
    ...
    if SERVER then
        ...
        timer.Simple(0.1, function()
            if self and self:IsValid() then
                ParticleEffectAttach("astral_smallgeyser_idle", PATTACH_POINT_FOLLOW, self, 0)
            end
        end)
    ...
    if SERVER then
        ...
        if self.Erupting then
            ...
            if not self.EmittingEruptionParticle then
                ParticleEffectAttach("astral_smallgeyser", PATTACH_POINT_FOLLOW, self, 0)
