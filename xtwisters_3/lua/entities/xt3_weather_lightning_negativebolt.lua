...
ENT.PrintName = "XTwisters3 Negative Lightning"
ENT.Category = "XT3"

local TestBoltParticleName = "astral_negativebolt_strikearc1"
local TestBoltImpactParticleName = "astral_lightning_strike"
...
function ENT:Initialize()
    ...
    elseif CLIENT then
        language.Add("xt3_weather_lightning_negativebolt", "Negative Lightning Bolt")
        killicon.Add("xt3_weather_lightning_negativebolt", "materials/KillIcons/ZapIcon.png", Color(255, 255, 255))
    end
end
...
function ENT:Strike(AimTorward, Length)
    ...
    if BoltTrace.Hit then
        ...
        for i = 1, math.random(0, 1 + math.random(0, 3)) do
        ...
        local LightLifetime = SuperRandom(0.15, 0.25)
        local LightSize = SuperRandom(15000, 30000) / 2
        ...
        if GetConVar("xtwisters3_lightningflashes"):GetBool() then
            LightType = 2
            LightBrightness = SuperRandom(350, 1000)
        end
        ...
        local Size = SuperRandom(1.5, 1.75)
        local DecalParams = {
            Decal = {
        ...
        EmitSound("ambient/ambience/rainscapes/thunder_close0" .. math.random(1, 4) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 120)
        ...
        util.ScreenShake(BoltTrace.HitPos, 100, 25, 1, 1700, true)
        local StrikeSize = 85
            ...
            if Entity:IsValid() and Entity ~= self and Entity:GetClass() ~= "gmod_ghost" and Entity:GetClass() ~= "sent_anim" and not Entity:IsWorld() and not Entity:IsWeapon() then
                ...
                if PhysObject:IsValid() then
                    if (Entity:IsPlayer() and Entity:GetMoveType() ~= MOVETYPE_NOCLIP) or Entity:IsNPC() then
                        ...
                        local Velocity = -(BoltTrace.HitPos - Entity:GetPos()):GetNormalized() * 250000 * (1 - math.min((BoltTrace.HitPos - Entity:GetPos()):Length() / StrikeSize, 1))
                    ...
                    elseif not Entity:IsPlayer() and not Entity:IsNPC() then
                        ...
                        PhysObject:ApplyForceCenter(-(BoltTrace.HitPos - Entity:GetPos()):GetNormalized() * 250000 * (1 - math.min((BoltTrace.HitPos - Entity:GetPos()):Length() / StrikeSize, 1)))
