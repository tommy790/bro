...
ENT.PrintName = "XTwisters3 Super Lightning"
ENT.Category = "XT3"

local TestBoltParticleName = "astral_superbolt_strikearc1"
local TestBoltImpactParticleName = "astral_superbolt_strike"
...
function ENT:Initialize()
    ...
    elseif CLIENT then
        language.Add("xt3_weather_lightning_superbolt", "Super Lightning Bolt")
        killicon.Add("xt3_weather_lightning_superbolt", "materials/KillIcons/ZapIcon.png", Color(255, 255, 255))
    end
end
...
function ENT:Strike(AimTorward, Length)
    ...
    if BoltTrace.Hit then
        ...
        for i = 1, math.random(0, 10) do
            local LastBranchEndPos = LightningPosition

            for Index = 1, math.random(0, 2) do
                local MaxDistance = 100000 / 4

                local ZVector = LightningPosition[3] - SuperRandom(0, MaxDistance / 4)
                local NextVectorRand = BoltTrace.HitPos + Vector(SuperRandom(-MaxDistance, MaxDistance), SuperRandom(-MaxDistance, MaxDistance), ZVector)

                XT3CreateConnectedParticle(TestBoltParticleName, LastBranchEndPos, NextVectorRand)
                LastBranchEndPos = NextVectorRand
            end
        end

        XT3ParticleEffect(TestBoltImpactParticleName, BoltTrace.HitPos, Angle(), self)

        local LightLifetime = SuperRandom(0.15, 0.25) * 3
        local LightSize = SuperRandom(15000, 30000) * 4
        ...
        if GetConVar("xtwisters3_lightningflashes"):GetBool() then
            LightType = 2
            LightBrightness = SuperRandom(350, 1000) * 10
        end
        ...
        local Size = SuperRandom(8.5, 8.75)
        local DecalParams = {
            Decal = {
        ...
        EmitSound("ambient/ambience/rainscapes/thunder_close0" .. math.random(1, 4) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 35)

        EmitSound("ambient/weather/thunder_distant_0" .. math.random(3, 5) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 100)
        EmitSound("ambient/weather/thunder_distant_0" .. math.random(3, 5) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 75)
        EmitSound("ambient/weather/thunder_distant_0" .. math.random(3, 5) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 50)
        EmitSound("ambient/weather/thunder_distant_0" .. math.random(3, 5) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 35)

        EmitSound("ambient/levels/labs/electric_explosion" .. math.random(1, 4) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 120)
        EmitSound("ambient/explosions/explode_" .. math.random(1, 4) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 120)
        EmitSound("ambient/explosions/explode_" .. 6 .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 120)
        
        -- EmitSound("ambient/energy/ion_cannon_shot" .. math.random(1, 3) .. ".wav", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 80)
        EmitSound("Explosions/LightningBlastExtreme" .. math.random(1, 3) .. ".mp3", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 100)
        EmitSound("Explosions/LightningBlastExtreme" .. math.random(1, 3) .. ".mp3", BoltTrace.HitPos, 0, CHAN_AUTO, 1, 0, 0, 50)

        util.ScreenShake(BoltTrace.HitPos, 1000, 25, 3, 50000, true)
        local StrikeSize = 1000
            ...
            if Entity:IsValid() and Entity ~= self and Entity:GetClass() ~= "gmod_ghost" and Entity:GetClass() ~= "sent_anim" and not Entity:IsWorld() and not Entity:IsWeapon() then
                ...
                if PhysObject:IsValid() then
                    if (Entity:IsPlayer() and Entity:GetMoveType() ~= MOVETYPE_NOCLIP) or Entity:IsNPC() then
                        ...
                        local BoltDamage = SuperRandom(850, 4500) * (1 - math.min((BoltTrace.HitPos - (Entity:GetPos() + Vector(0, 0, 30))):Length() / StrikeSize, 1))
                        Entity:Ignite(SuperRandom(1, 5) * DamageMult)

                        local Velocity = -(BoltTrace.HitPos - Entity:GetPos() + (VectorZAxis*25)):GetNormalized() * 1050000 * (1 - math.min((BoltTrace.HitPos - Entity:GetPos()):Length() / StrikeSize, 1))
                        ...
                        XT3CauseDamage(self, self, Entity, BoltDamage * DamageMult, DMG_DISSOLVE)
                    elseif not Entity:IsPlayer() and not Entity:IsNPC() then
                        local BoltDamage = SuperRandom(1000, 6000) * (1 - math.min((BoltTrace.HitPos - Entity:GetPos()):Length() / StrikeSize, 1))
