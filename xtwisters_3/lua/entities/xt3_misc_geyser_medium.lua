...
ENT.DisplayName = "Medium Geyser"
ENT.PrintName = "XTwisters3 Medium Large"
...
ENT.GeyserSize = 125
ENT.EruptSize = 125
ENT.EruptStrength = 145

local GeyserScale = Vector(3.5, 4.5, 1)
    ...
    if SERVER then
        ...
        timer.Simple(0.1, function()
            if self and self:IsValid() then
                ParticleEffectAttach("astral_medigeyser_idle", PATTACH_POINT_FOLLOW, self, 0)
            end
        end)
    ...
    if SERVER then
        ...
        if self.Erupting then
            ...
            if not self.EmittingEruptionParticle then
                ParticleEffectAttach("astral_medigeyser", PATTACH_POINT_FOLLOW, self, 0)
