...
local function populate()
    ...
    spawnmenu.AddToolMenuOption("V-NPCs", "Status & Traits", "vnpcs_belly_physics", "Belly Physics & Digestion", "", "", function(panel)
        ...
        panel:CheckBox("Shape-aware belly (bounding-box deform)", "vnpcs_shape_deform")
        panel:ControlHelp("Belly shape adapts to the actual size/shape of what's inside, and widens/splits into rows for multiple prey.")
        ...
        panel:CheckBox("Weight-Painted Belly Shape (GPU)", "vnpcs_weight_paint_enabled")
        panel:CheckBox("Metaball Merged Surface", "vnpcs_weight_paint_metaballs")
        panel:CheckBox("Fetal-Curl Multi-Blobs", "vnpcs_weight_paint_curl")
        panel:CheckBox("Volume-Correct Radius", "vnpcs_weight_paint_volume")
        panel:CheckBox("Gravity Sag", "vnpcs_weight_paint_sag")
        panel:CheckBox("Client Blob Lerp", "vnpcs_weight_paint_lerp")
        panel:CheckBox("Per-Prey Lumps / Field", "vnpcs_weight_paint_lumps")
        panel:NumSlider("Field Amplitude", "vnpcs_weight_paint_amp", 0, 3, 2)
        panel:CheckBox("Asymmetric Bones", "vnpcs_weight_paint_asymmetry")
        panel:ControlHelp("Metaball field merges adjacent prey into one continuous bulge. Curl adds head/limb sub-blobs. Volume grows with mass^(1/3). Sag drops a heavy belly.")
