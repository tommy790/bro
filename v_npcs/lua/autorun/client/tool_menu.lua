...
local function populate()
    ...
    spawnmenu.AddToolMenuOption("V-NPCs", "Status & Traits", "vnpcs_belly_physics", "Belly Physics & Digestion", "", "", function(panel)
        ...
        panel:CheckBox("Shape-aware belly (bounding-box deform)", "vnpcs_shape_deform")
        panel:ControlHelp("Belly shape adapts to the actual size/shape of what's inside, and widens/splits into rows for multiple prey, instead of inflating as one uniform blob.")
        panel:NumSlider("Shape Intensity", "vnpcs_shape_intensity", 0.5, 3, 2)
        panel:NumSlider("Body Nest Tightness", "vnpcs_shape_nest", 0.5, 1, 2)
        panel:NumSlider("Multi-Body Silhouette", "vnpcs_shape_multibody", 1, 2.5, 2)
        panel:NumSlider("Max Width Bias", "vnpcs_shape_max_width", 1.2, 4.5, 2)
        panel:NumSlider("Max Depth Bias", "vnpcs_shape_max_depth", 1.2, 3.5, 2)
        panel:ControlHelp("Intensity/multi-body make 2+ prey read clearly as multiple bodies. Nest tightness controls how much shoulders overlap when packing.")
        ...
        panel:ControlHelp("Classic belly model uses GetBellyShapeVector packing (HalfExtents shelf pack). GPU belly also uses measured body-part blobs when enabled.")
        ...
        do
            ...
            local btn3 = panel:Button("Bounding-Box Shape Status")
            btn3.DoClick = function()
                RunConsoleCommand("vnpcs_shape_status")
            end
        end
    end)
