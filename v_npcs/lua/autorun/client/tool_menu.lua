...
local function populate()
    spawnmenu.AddToolMenuOption("V-NPCs", "Personalization", "vnpcs_settings", "Settings", "", "", function(panel)
        ...
        panel:CheckBox("Torso Hull Mesh (GPU belly)", "vnpcs_gpu_belly_torso_hull")
        panel:CheckBox("Hide Belly Entity (GPU Only)", "vnpcs_gpu_belly_hide_entity")
        panel:CheckBox("Torso Bone Inflation (Belly Bonescale)", "vnpcs_gpu_belly_bonescale")
        panel:ControlHelp("ON by default: the GPU mesh creates the belly (torso hull + hidden belly model) with all procedural deform. Bone inflation is opt-in - leave off unless you want the character's own torso bones to swell too.")
        ...
        panel:Help("Gulp is one traveling ridge on the neck/chest.")
    ...
    spawnmenu.AddToolMenuOption("V-NPCs", "Status & Traits", "vnpcs_belly_physics", "Belly Physics & Digestion", "", "", function(panel)
        ...
        panel:ControlHelp("Prey now simulate as live masses inside the belly: they bounce, struggle, slosh and slow the predator down.")
    ...
    spawnmenu.AddToolMenuOption("V-NPCs", "Status & Traits", "vnpcs_v07", "v0.7 Systems", "", "", function(panel)
        panel:ClearControls()
        panel:Help("Real-Time Dynamic Body Matrix (No More Rigging)")
        panel:CheckBox("Body Expansion (Chest/Hips/Thighs/Arms)", "vnpcs_body_expansion_enabled")
        panel:ControlHelp("OFF by default: the GPU mesh creates the belly. Enable only if you want the character's own bones to inflate as well (may look odd).")
        panel:NumSlider("Expansion Amplitude", "vnpcs_body_expansion_amp", 0, 2, 2)
        panel:CheckBox("Expand Prey Citizens Too", "vnpcs_body_expansion_prey")
        panel:Help("\\nLayered Cloth Tear\\n")
        panel:CheckBox("Fabric Shell + Stress Tears", "vnpcs_cloth_tear_enabled")
        panel:NumSlider("Tear Stress Threshold", "vnpcs_cloth_tear_stress", 0.1, 1, 2)
        panel:NumSlider("Rip Threshold", "vnpcs_cloth_tear_rip", 0.3, 1.2, 2)
        panel:ControlHelp("The belly reads stress from bloat: fabric appears strained, then rips open along seams at Monster/extreme bloat.")
        panel:Help("\\nAdaptive Environment AI\\n")
        panel:CheckBox("Prop Analysis (Tables/Beds/Vents)", "vnpcs_env_ai_enabled")
        panel:NumSlider("Env Scan Radius", "vnpcs_env_scan_radius", 200, 2000, 0)
        panel:CheckBox("Env AI Debug Overlay", "vnpcs_env_debug")
        panel:Help("\\nAcoustical Sound Porting\\n")
        panel:CheckBox("Acoustics Engine", "vnpcs_acoustics_enabled")
        panel:CheckBox("Muffle Internal Sounds", "vnpcs_acoustics_muffle")
        panel:CheckBox("Sub-Bass Echo", "vnpcs_acoustics_echo")
        panel:CheckBox("Heavy Footstep Thumps", "vnpcs_acoustics_thump")
        do
            local btn = panel:Button("v0.7 Systems Status")
            btn.DoClick = function()
                RunConsoleCommand("vnpcs_body_matrix_status")
                RunConsoleCommand("vnpcs_env_ai_status")
                RunConsoleCommand("vnpcs_acoustics_status")
            end
        end
    end)
