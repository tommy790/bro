...
function ENT:SetBellySize()
    ...
    -- Lightweight multi-body hints for client deform (lerp/side push/fatfolds).
    local occupants = 0
    if istable(self.Prey) then
        for _, info in ipairs(self.Prey) do
            if info and info.HalfExtents then
                local remaining = 1
                if info.Absorbing and info.TrueValue and info.TrueValue > 0 then
                    remaining = math.Clamp(info.Value / info.TrueValue, 0, 1)
                end
    ...
    -- Slight off-center bias when 2+ bodies so client silhouette isn't a centered balloon.
    local sideBias = 0
    if occupants >= 2 then
        sideBias = math.Clamp((occupants - 1) * 0.12, 0, 0.35)
    end
    self:SetNWFloat("BellyShapeBias", sideBias)
    self:SetNWInt("BellyOccupants", occupants)
